...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Data Pegawai</h1>
            <p class="text-xs text-slate-500 mt-1">Perbarui data kepegawaian dan penyesuaian sisa kuota cuti.</p>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="jatah_cuti" class="block text-xs font-bold uppercase text-slate-700 mb-1">
                        Total Jatah Cuti (Hari) <span class="text-rose-500">*</span>
                    </label>
                    <input type="number" name="jatah_cuti" id="jatah_cuti" 
                           value="{{ old('jatah_cuti', $pegawai->jatah_cuti) }}" 
                           min="0" max="30" 
                           class="w-full rounded-xl border-slate-300 p-3 bg-slate-50 text-slate-900 text-sm font-bold focus:ring-blue-500 focus:border-blue-500" required>
                </div>

                <div>
                    <label for="sisa_cuti" class="block text-xs font-bold uppercase text-slate-700 mb-1">
                        Sisa Kuota Cuti (Hari) <span class="text-rose-500">*</span>
                    </label>
                    <input type="number" name="sisa_cuti" id="sisa_cuti" 
                           value="{{ old('sisa_cuti', $pegawai->sisa_cuti) }}" 
                           min="0" max="30" 
                           class="w-full rounded-xl border-slate-300 p-3 bg-slate-50 text-slate-900 text-sm font-bold text-emerald-700 focus:ring-blue-500 focus:border-blue-500" required>
                </div>
            </div>
